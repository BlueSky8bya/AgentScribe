...
- **Phase 3C-1**: Multi-provider **foundation** — `ProviderAdapter` interface, Capability Matrix, `/api/providers`, provider/tier allowlist, key-availability, mock adapters, pricing extension (`price_status`), Contract Canary harness (mock). OpenAI stays the only `live` provider; Gemini/Claude/DeepSeek are `mock`/`experimental` (not user-selectable for real generation).
- **Phase 3C-2**: Live provider rollout. Claude (`@anthropic-ai/sdk`) and Gemini (`@google/genai`) get **live adapters** but stay `user_selectable=false` (gated) until they pass the Contract Canary — `adapter_mode=live` is separate from `user_selectable`/`can_generate_real_output` (the UI/routing gate). DeepSeek stays `mock`/placeholder until its official model names + data policy are re-confirmed (last step). Canary has hard caps (≤30 calls, ≤$5/run). First-request latency (Claude schema-compile) is logged separately. Data policy: OpenAI/Anthropic API + Gemini paid-tier inputs are not used for training by default; only public/writer-safe payload (`payload_class`) ever leaves the server.
- **Phase 4**: Writer episode generation. `/api/write` takes a writer-safe `WriterContract` (public group only, via `buildWriterContract` + `assertNoPrivateLeak`), generates ONE Korean episode body through OpenAI/Claude (user_selectable providers only; Gemini/DeepSeek -> `provider_unavailable`). `EpisodeDraft` carries `status` (draft|failed) + `commit_status` (generated|user_saved|discarded). No deterministic prose fallback: on persistent failure -> `status="failed"`, never fabricated, never saved. Drafts persist only when the user saves (`user_saved`). Korean validated (Hangul-ratio heuristic); `created_at` server-injected (tests use a fixed clock).
- Phase 5+: Creative Review, LLM semantic Immersion Gates, NMK full, operational architecture (DB/UI/latency).
